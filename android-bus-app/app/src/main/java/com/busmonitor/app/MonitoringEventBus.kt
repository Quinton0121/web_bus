...
object MonitoringEventBus {
    ...
    private val _isMonitoring = MutableSharedFlow<Boolean>(replay = 1, extraBufferCapacity = 1)
    val isMonitoring = _isMonitoring.asSharedFlow()

    suspend fun postLog(message: String) {
        _logs.emit(message)
    }

    suspend fun setMonitoringState(active: Boolean) {
        _isMonitoring.emit(active)
    }
}
